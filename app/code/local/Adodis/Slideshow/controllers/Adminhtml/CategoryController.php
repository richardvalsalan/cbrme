...
class Adodis_Slideshow_Adminhtml_CategoryController extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() {
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addContent($this->getLayout()->createBlock('slideshow/adminhtml_category_edit'))
				->_addLeft($this->getLayout()->createBlock('slideshow/adminhtml_category_edit_tabs'));
    ...
    public function massDeleteAction() {
        $slideshowIds = $this->getRequest()->getParam('category');
        if(!is_array($slideshowIds)) {
			Mage::getSingleton('adminhtml/session')->addError(Mage::helper('adminhtml')->__('Please select item(s)'));
        } else {
            try {
                foreach ($slideshowIds as $slideshowId) {
                    $slideshow = Mage::getModel('slideshow/category')->load($slideshowId);
                    $slideshow->delete();
                }
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($slideshowIds)) {
            ...
            try {
                foreach ($slideshowIds as $slideshowId) {
                    $slideshow = Mage::getSingleton('slideshow/category')
                        ->load($slideshowId)
    ...
    public function exportCsvAction()
    {
        ...
        $content    = $this->getLayout()->createBlock('slideshow/adminhtml_category_grid')
            ->getCsv();
