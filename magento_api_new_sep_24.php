...
}else if(isset($_GET["task"]) && $_GET["task"] == "doUpdateAddress")
{
	//$addr["id"]= $_GET["id"];
	$addr["user_id"]= $_GET["user_id"];
//	$addr["addr_type"]= $_GET["addr_type"]; //new or update
	$addr["city"]= $_GET["city"];
	$addr["company"]= $_GET["company"];
	$addr["country_id"]= $_GET["country_id"];
	$addr["fax"]= $_GET["fax"];
	$addr["firstname"]= $_GET["firstname"];
	$addr["lastname"]= $_GET["lastname"];
	$addr["middlename"]= $_GET["middlename"];
	$addr["postcode"]= $_GET["postcode"];
	$addr["region"]= $_GET["region"];
	$addr["street"]= $_GET["street"];
	$addr["telephone"]= $_GET["telephone"];
	$addr["default_billing"]= $_GET["default_billing"];
	$addr["default_shipping"]= $_GET["default_shipping"];
...
}else if(isset($_GET["task"]) && $_GET["task"] == "placeOrder55")
{
...
//	print_r ($totals);exit;
	...
	$result["cart"]["subtotal"] = strip_tags ( Mage::helper('core')->currency($totals["subtotal"]->getValue()));
	...
	if($shippingCode != '')
	{
		...
		$result["cart"]["shipping"]["rate"] = strip_tags ( Mage::helper('core')->currency($totals["shipping"]->getValue()));
	...
	// $quoteSession->setStoreId($storeId);
	// $quoteSession->setCustomerId($userid);
   	$_newQuoteid = $quote->save()->getId();

   	$newQuote = Mage::getModel('sales/quote')->load($_newQuoteid);

   	echo "<pre>";
   	print_r($newQuote->toArray());
...
/*		$quoteId = $quote->getId();
	    ...
	    $orderObj = $convertQuoteObj->addressToOrder($quote->getShippingAddress()->addData($shippingAddress))->setStore($store);
	    $orderPaymentObj = $convertQuoteObj->paymentToOrderPayment($quotePaymentObj);
	    	    // exit;
...
} else if(isset($_GET["task"]) && $_GET["task"] == "placeOrder")
{
	//$cartDataJSON = '{"product":[{"id":"83","qty":"1","confAttr":[{"id":"501","option":"36"},{"id":"502","option":"37"}]},{"id":"93","qty":"1","confAttr":[{"id":"502","option":"38"}]},{"id":"98","qty":"1","confAttr":[{"id":"502","option":"41"}]}]}';
	$cartDataJSON = '{"product":[{"id":"83","qty":"2","confAttr":[{"id":"501","option":"36"},{"id":"502","option":"37"}]}]}';
//	$cartDataJSON = '{"product":[{"id":"26","qty":"1","custAttr":[{"id":"2","value":"4"}]}]}';
...
}else if(isset($_GET["task"]) && $_GET["task"] == "doRating")
{
